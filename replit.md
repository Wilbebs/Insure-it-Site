...
- **Navigation**: Glass morphism navbar with logo (animated WebM shield with WebKit fallback for static image). The desktop navbar (`lg:`) is `fixed left-1/2 -translate-x-1/2`, sized to its content when scrolled (rounded pill) and full-width when at the top of the page. To accommodate the Client Center link without overflowing the viewport on hover, the inner flex uses `gap-6` when scrolled (was `gap-12`) and the hovered `paddingRight` is `5rem` (was `4rem`). When scrolled and not hovered the social icons stack with `-space-x-6` and the language toggle is hidden; on hover, the icons spread (`gap-2`), the language toggle appears with `ml-1`, and the pill grows to fit — the gap reduction prevents the expanded width from exceeding the viewport, which would clip the language toggle.

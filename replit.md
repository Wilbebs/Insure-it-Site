...
- **Client Center Page** (`/client-center`): Full-viewport embed of the EZLynx Customer Service portal (`https://customerservice.agentinsure.com/EzlynxCustomerService/insure/Account/LogIn`) for existing customers to log in and manage policies. Renders only the iframe — no Insure-IT navbar, footer, hero, card wrapper, or "Not a client yet?" CTA — to avoid the window-within-a-window look the user disliked. The page wrapper is `fixed inset-0 bg-white` and the iframe fills it via `w-full h-full border-0`. A `<noscript>` fallback link is layered on top for users with JS disabled. Page is `noindex` (in metadata robots) since it's a portal landing, not SEO content. Users navigate back via browser back button or by re-entering the main site at any other URL. Discoverable via navbar (desktop label "Client Center", mobile short label "Clients" / "Clientes") and footer Quick Links.

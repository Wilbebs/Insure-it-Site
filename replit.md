...
- **Client Center Page** (`/client-center`): Full-page embed of the EZLynx Customer Service portal (`https://customerservice.agentinsure.com/EzlynxCustomerService/insure/Account/LogIn`) for existing customers to log in and manage policies. Uses a fixed-height (1600px) iframe inside a card wrapper — not a modal — because the portal is too tall for a modal experience. Includes a "Not a client yet?" CTA at the bottom that opens the QuoteModal so prospects who land on this page can still convert. Page is `noindex` (in metadata robots) since it's a portal landing, not SEO content. Discoverable via navbar (desktop label "Client Center", mobile short label "Clients" / "Clientes") and footer Quick Links.

...
- **QuoteModal**: Embeds the EZLynx Consumer Quoting iframe (`https://www.agentinsure.com/compare/auto-insurance-home-insurance/insure/quote.aspx`) inside the existing Dialog shell. Loading overlay (spinner + `t.quote.loadingQuote` text) shows until iframe `onLoad` fires, with an 8s fallback timer. Iframe sandbox: `allow-scripts allow-forms allow-same-origin allow-popups allow-popups-to-escape-sandbox`. **Why scaling, not fluid width:** EZLynx's page is not responsive — even at 768px it has internal horizontal scroll and overlapping header/logo. So the iframe is rendered at a fixed natural size of `1024 × 1500` (`NATURAL_WIDTH`/`NATURAL_HEIGHT`), which is EZLynx's true desktop layout width where everything fits cleanly, and CSS-scaled down to fit the available container via `transform: scale(scale)` with `transform-origin: top left`. **Measurement strategy:** rather than measuring the wrapper's `clientWidth` (which is unreliable inside Radix's CSS grid because the 1024px iframe pushes its grid track wider than the modal), `scale` is computed from `window.innerWidth` directly. The formula: mobile (`vw < 640`) uses `vw` as available width (modal is flush, no padding); desktop uses `min(vw, 768) - 48` (modal is `max-w-3xl` with default `p-6` of 24px each side). Resulting scale = `clamp(0.2, 1, available / 1024)`. Wrapper height = `NATURAL_HEIGHT * scale` so there is no empty space below the scaled iframe. The wrapper still has `min-w-0` and `overflow-hidden` so any sub-pixel mismatch can't bust the layout. State updates on `resize` and `orientationchange`, with initial value computed during `useState` initializer to avoid a first-paint flash at scale=1. Desktop: `max-w-3xl max-h-[90vh]` with the bilingual header visible. Mobile (≤640px): edge-to-edge sheet — `w-screen max-w-[100vw] h-[100dvh] p-0 gap-0 border-0 rounded-none`, the DialogHeader is `sr-only` (retained in DOM for `aria-describedby`), and a custom floating `DialogClose` X button (`fixed top-2 right-2 z-[60]`, white circle with shadow) replaces the small built-in close so the scaled iframe gets every available pixel. Modal scrolls vertically through the scaled iframe height. Legacy form/schema/success state/`/api/contact` mutation removed from this component (chatbot still uses `/api/contact` independently).

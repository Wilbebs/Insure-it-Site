...
- **UI/UX Decisions**: Glass-morphism navigation (scrolled-pill + full-width-bar dual mode; tighter `gap-3` between groups and `gap-2` between center nav links at lg breakpoint, looser `gap-6` / `gap-3` at xl+; in scrolled pill the third nav link uses `clientCenterShort` ("Clients" / "Clientes") instead of the full label so Spanish "Centro de Clientes" doesn't push the language toggle off the right edge during hover-expansion, while the full-width bar at the top of the page still shows the long form), device-responsive hero section with a draggable "Window Card," and auto-cycling testimonials carousel.

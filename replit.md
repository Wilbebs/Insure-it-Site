...
- **Hero Static Image**: Uses `<picture>` element with `<source media="(max-width: 640px)" srcSet="/images/heroimage_mobile.png">` — phones download ONLY the portrait Jacksonville bridge image, desktop/tablet download ONLY `heroimage1.webp`. `<picture>` loads eagerly (SEO, fetchPriority="high"); video lazy-loads after `window.onload`. Video `poster=` also switches per device. Both images in `public/images/`.

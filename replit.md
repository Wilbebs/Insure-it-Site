...
- **Client Center Page** (`/client-center`): Embed of the EZLynx Customer Service portal (`https://customerservice.agentinsure.com/EzlynxCustomerService/insure/Account/LogIn`) for existing customers. Layout: page wrapper uses the site-wide `dot-pattern` over `bg-muted` so the polka dot texture shows around the iframe card (matches About page aesthetic). The standard glass-morphism `Navigation` is fixed at top. The EZLynx iframe is full-page width (no max-width, no card chrome) — its wrapper is `relative w-full bg-muted dot-pattern overflow-hidden` so the polka dots become the iframe's visible background. The iframe itself is `bg-transparent`, and because EZLynx's own page body has no opaque background, the polka dot pattern shows through wherever EZLynx doesn't paint its own UI (the empty area around the sign-in form, the right-column header/footer space, etc.). The whole client center page therefore reads as one continuous polka-dotted surface with EZLynx's blue welcome panel and white form fields floating on top. Iframe height is fixed at `1300px` mobile / `760px` desktop — sized to EZLynx's actual content height in each layout (mobile stacks the welcome and sign-in panels vertically, desktop puts them side-by-side). Main wrapper padding is `pt-24 sm:pt-28 pb-24 sm:pb-24` so the iframe clears the navbar at top and the floating CTA pill at bottom. The "Not a client yet?" CTA is rendered as a floating frosted-glass pill (`fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-40`, `bg-white/95 backdrop-blur-md`, `rounded-full`) overlapping the bottom of the iframe card — shield icon + headline + Get a Quote button in one row. The outer wrapper is `pointer-events-none` and the inner pill `pointer-events-auto` so empty space around the pill doesn't block clicks on the iframe. No site footer (would push the iframe out of the viewport). A `<noscript>` fallback link is layered inside the iframe wrapper for users with JS disabled. Page is `noindex` (in metadata robots) since it's a portal landing, not SEO content. Discoverable via navbar (desktop label "Client Center", mobile short label "Clients" / "Clientes") and footer Quick Links on every other page.

...
- **Client Center Page** (`/client-center`): Embed of the EZLynx Customer Service portal (`https://customerservice.agentinsure.com/EzlynxCustomerService/insure/Account/LogIn`) for existing customers. Layout: page wrapper uses the site-wide `dot-pattern` over `bg-muted` so the polka dot texture shows around the iframe card (matches About page aesthetic). The standard glass-morphism `Navigation` is fixed at top. The EZLynx iframe lives in a centered white card (`max-w-6xl`, `rounded-2xl sm:rounded-3xl`, `shadow-2xl`, `ring-1 ring-slate-200`) inside a container with `pt-28 sm:pt-32 pb-32 sm:pb-28` so the polka dot bg is visible around all four edges of the card. Iframe height is fixed at `1300px` mobile / `760px` desktop — sized to EZLynx's actual content height in each layout (mobile stacks the welcome and sign-in panels vertically, desktop puts them side-by-side). These values were chosen empirically by trial: anything smaller cropped content; anything larger left dead white space below the form. The user scrolls the page (not the iframe) for any overflow. The "Not a client yet?" CTA is rendered as a floating frosted-glass pill (`fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-40`, `bg-white/95 backdrop-blur-md`, `rounded-full`) overlapping the bottom of the iframe card — shield icon + headline + Get a Quote button in one row. The outer wrapper is `pointer-events-none` and the inner pill `pointer-events-auto` so empty space around the pill doesn't block clicks on the iframe. No site footer (would push the iframe out of the viewport). A `<noscript>` fallback link is layered inside the iframe wrapper for users with JS disabled. Page is `noindex` (in metadata robots) since it's a portal landing, not SEO content. Discoverable via navbar (desktop label "Client Center", mobile short label "Clients" / "Clientes") and footer Quick Links on every other page.

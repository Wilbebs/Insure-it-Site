...
- **Mobile hero image** uses `heroimage_mobile_v3.webp` (183 KB, 720×1560, q=85). It's a still extracted from the mobile hero video (`herovid_mobile.mp4`) at the 8.5s mark using ffmpeg, then downscaled from the video's native 1080×2340 to a retina-sweet-spot 720×1560. This gives visual continuity — phones see the exact frame the video would have settled on, so dropping the video on mobile costs no visual context. Older variants (`heroimage_mobile.webp` 173 KB and `heroimage_mobile_v2.webp` 84 KB) remain on the CDN for OG/social previews. Both desktop and mobile preload tags in `app/layout.tsx` use conditional `media` queries so each device only fetches its variant.

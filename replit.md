...
- **QuoteModal**: Embeds the EZLynx Consumer Quoting iframe (`https://www.agentinsure.com/compare/auto-insurance-home-insurance/insure/quote.aspx`) inside the existing Dialog shell. Loading overlay (spinner + `t.quote.loadingQuote` text) shows until iframe `onLoad` fires, with an 8s fallback timer. Iframe sandbox: `allow-scripts allow-forms allow-same-origin allow-popups allow-popups-to-escape-sandbox`. Desktop: `max-w-3xl max-h-[90vh]` with the bilingual header visible and iframe at fixed `h-[1500px]` (modal scrolls). Mobile (≤640px): true edge-to-edge sheet — `w-screen max-w-[100vw] h-[100dvh] p-0 gap-0 border-0 rounded-none overflow-hidden flex flex-col`. The DialogHeader is `sr-only` on mobile (kept in DOM for `aria-describedby` accessibility) and a custom floating `DialogClose` X button (`fixed top-2 right-2 z-[60]`, white circle with shadow) replaces the small built-in close. Iframe is `w-full h-full` on mobile (fills viewport, EZLynx handles its own internal scroll). No CSS `transform: scale()` or zoom — iframe reflows naturally at the actual viewport width. Iframe has `key={reloadKey}` that increments on `orientationchange` (and on debounced `resize` when crossing the 640px breakpoint) to force a clean remount when EZLynx would otherwise cache its initial layout width. Legacy form/schema/success state/`/api/contact` mutation removed from this component (chatbot still uses `/api/contact` independently).

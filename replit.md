...
- **Hero Video**: Dual-source via CloudFront CDN. Mobile phones (≤640px) receive `herovid_mobile.mp4` (portrait 9:19.5); desktop/tablet receive `herovid1.mp4` (landscape). Both at `https://d3gkfgi9drj9kb.cloudfront.net/video-assets/`. Three-layer fallback: CSS bg → `<img>` (always in DOM for SEO) → `<video>` fades in on `canPlay`. Source selected via JS (`isMobilePhone` state, set once on `window.onload` by checking `window.innerWidth <= 640`) — NOT via `media` attribute which is unsupported for `<video><source>`.
